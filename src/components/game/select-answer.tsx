...
type SelectAnswerProps = {
  step: 'league' | 'team' | 'player'
  setStep: React.Dispatch<React.SetStateAction<'league' | 'team' | 'player'>>
  onChange: (data: PlayerItem) => void
}

export default function SelectAnswer({ step, setStep, onChange }: SelectAnswerProps) {
  ...
  return (
    <div ref={containerRef} className='grid grid-cols-12 gap-6 relative'>
      {/* Chọn giải đấu */}
      {(step === 'league' || !isMobile) && (
        <div className='col-span-12 md:col-span-4 space-y-4'>
          <h3 className='text-center font-semibold tracking-tight'>Giải đấu</h3>
          <div className='flex md:hidden justify-end'>
            <Button variant='outline' size='sm' onClick={() => setStep('team')}>
              Câu lạc bộ
              <ChevronRight />
            </Button>
          </div>
      ...
      {(step === 'team' || !isMobile) && (
        <div className='col-span-12 md:col-span-4 space-y-4'>
          <h3 className='text-center font-semibold tracking-tight'>Câu lạc bộ</h3>
          <div className='flex md:hidden justify-between items-center space-x-4'>
            <Button variant='outline' size='sm' onClick={() => setStep('league')}>
              <ChevronLeft />
              Giải đấu
            </Button>
            <Button variant='outline' size='sm' onClick={() => setStep('player')}>
              Cầu thủ
              <ChevronRight />
            </Button>
          </div>
